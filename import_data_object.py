...
class run_parameter:
    ...
    def create_scenarios(self):
        match self.scen:
            ...
            case 3:
                self.electrolyser = pd.DataFrame({
                    "name": ["electrolyser Bornholm", "electrolyser_NS1", "electrolyser_NS2", "e1", "e2", "e3", "e4",
                             "e5",
                             "e6", "e7", "e8", "e9", "e10", "e11", "e12", "e13", "e14"],
                    "bus": [521, 522, 523, 403, 212, 209, 170, 376, 357, 279, 103, 24, 357, 62, 467, 218, 513],
                    "cost": [645000, 645000, 645000, 450000, 450000, 450000, 450000, 450000, 450000, 450000, 450000,
                             450000,
...
class model_data:
    def __init__(self, create_res, reduced_ts, export_files, run_parameter):
        ...
        load_raw = pd.read_csv(run_parameter.import_folder + "PyPSA_elec1024/load.csv", index_col=0).reset_index(
            drop=True)
        ...
        busses_filtered = busses_raw[
            busses_raw["country"].isin(run_parameter.country_selection)].reset_index().reset_index()
        ...
        solar_filtered = solar_matched[["p_nom_max", "carrier", "index", "country", "bidding_zone"]].reset_index(
            drop=True)
        solar_filtered.columns = ["max", "type", "bus", "country", "bidding_zone"]
        solar_filtered = solar_filtered.replace({"solar": "Solar"})
        wind_filtered = wind_matched[["p_nom_max", "carrier", "index", "country", "bidding_zone"]].reset_index(
            drop=True)
        ...
        lines_matched = lines_raw.merge(self.nodes[["index", "old_index"]], how="left", left_on="bus0",
                                        right_on="old_index")
        lines_matched = lines_matched.merge(self.nodes[["index", "old_index"]], how="left", left_on="bus1",
                                            right_on="old_index")
        ...
        lines_DC_matched = links_raw.merge(self.nodes[["index", "old_index"]], how="left", left_on="bus0",
                                           right_on="old_index")
        lines_DC_matched = lines_DC_matched.merge(self.nodes[["index", "old_index"]], how="left", left_on="bus1",
                                                  right_on="old_index")
        ...
        # get new renewables
        self.res_series, self.share_solar, self.share_wind = new_res_mapping(self, old_solar=solar_filtered,
                                                                             old_wind=wind_filtered,
                                                                             create_res_mapping=create_res,
                                                                             location=run_parameter.import_folder,
                                                                             query_ts=False)
        ...
        def clear_dam_ts(ts_raw, countries):
            ...
            NO_SE = filtered[
                filtered.columns.intersection(["NO1", "NO2", "NO3", "NO4", "NO5", "SE1", "SE2", "SE3", "SE4"])]
        ...
        ror_ts_T = ror_ts.T
        ror_bus_ts_matrix_nSE = ror_aggregated[~ror_aggregated["country"].isin(["SE"])].merge(ror_ts_T, how="left",
                                                                                              left_on="bidding_zone",
                                                                                              right_index=True).drop(
            ["P_inst", 'bidding_zone', "country"], axis=1).T
        ror_bus_ts_matrix_SE = ror_aggregated[ror_aggregated["country"].isin(["SE"])].merge(ror_ts_T, how="left",
                                                                                            left_on="country",
                                                                                            right_index=True).drop(
            ["P_inst", 'bidding_zone', "country"], axis=1).T
        ...
        self.storage = PHS.rename(
            columns={'P_inst': 'Pmax_out', 'pumping_MW': 'Pmax_in', 'storage_capacity_MWh': 'capacity'}).reset_index(
            drop=True)

        self.dispatchable_generators = pd.concat([conventionals, dam_unlimited], axis=0).reset_index(drop=True)[
            ["name", "type", "mc", "efficiency", "co2_fac", "P_inst", "bus", "bidding_zone"]]
    ...
    def conv_scaling_country_specific(self):  # ??? was macht das?
        ...
        tyndp_installed_capacity["generator"].replace(
            {"otherres": "biogas", "other": "gas", "ccgt": "CCGT", "ocgt": "OCGT", "gas": "OCGT"}, inplace=True)
        ...
        dispatchable_generators = {i: get_conventional_yearly(tyndp_values=tyndp_installed_capacity_regrouped,
                                                              df_2020_capacity_bz=conventional_fossil,
                                                              df_2020_capacity_bz_grouped=conventional_fossil_grouped,
                                                              conventional_h20=conventional_h20, year=year, i=i,
                                                              CO2_price=self.CO2_price) for i, year in
                                   zip([0, 1, 2], [2030, 2035, 2040])}
    ...
    def future_values(self):
        open_entrance_dataset = pd.read_csv("data/PyPSA_elec1024/openentrance-v01-al-2022_05_10.csv").fillna(0).iloc[
                                :-1, :]
        ...
        open_entrance_split_copy["Technology"] = open_entrance_split_copy["Technology"].replace(
            {"P_Biomass": "Biomass", "CHP_Biomass_Solid_CCS": "Biomass", "CHP_Biomass_Solid": "Biomass",
             "RES_Hydro_Large": "Hydro", "RES_Hydro_Small": "Hydro", 'RES_PV_Utility_Avg': "Solar",
             'RES_PV_Utility_Opt': "Solar",
             'Res_PV_Utility_Tracking': "Solar", "RES_Wind_Onshore_Avg": "Wind_onshore",
             'RES_Wind_Onshore_Opt': "Wind_onshore",
             'RES_Wind_Offshore_Deep': "Wind_offshore", 'RES_Wind_Offshore_Transitional': "Wind_offshore",
             'RES_Wind_Offshore_Shallow': "Wind_offshore",
             'RES_Wind_Onshore_Inf': "Wind_onshore", "RES_Geothermal": "Geothermal", "P_Coal_Hardcoal": "Coal",
        ...
        open_entrance_split["Technology"] = open_entrance_split["Technology"].replace(
            {"P_Biomass": "Biomass", "CHP_Biomass_Solid_CCS": "Biomass", "CHP_Biomass_Solid": "Biomass",
             "RES_Hydro_Large": "Hydro", "RES_Hydro_Small": "Hydro",
             'RES_PV_Utility_Avg': "Solar", 'RES_PV_Utility_Opt': "Solar", 'Res_PV_Utility_Tracking': "Solar",
             "RES_Wind_Onshore_Avg": "Wind",
             'RES_Wind_Onshore_Opt': "Wind", 'RES_Wind_Offshore_Deep': "Wind", 'RES_Wind_Offshore_Transitional': "Wind",
        ...
        self.scaling_res = open_entrance_split[open_entrance_split["renewable"] == True].groupby(
            ["Technology", "Region", "Year"], sort=False).sum().sort_index()
        self.scaling_res_separate = open_entrance_split_copy[open_entrance_split["renewable"] == True].groupby(
            ["Technology", "Region", "Year"], sort=False).sum().sort_index()

        conventionals = open_entrance_split[open_entrance_split["renewable"] == False].groupby(
            ["Technology", "Region", "Year"], sort=False).sum()
    ...
    def tyndp_values(self, path, bidding_zone_encyc):
        ...
        tyndp_installed_capacity["node"] = tyndp_installed_capacity["node"].replace(
            {"DKE1": "DK1", "DKW1": "DK2", "NOM1": "NO3", "NON1": "NO4", "NOS0": "NO1", "SE01": "SE1", "SE02": "SE2",
             "SE03": "SE3", "SE04": "SE4"})
        tyndp_installed_capacity = tyndp_installed_capacity[tyndp_installed_capacity["node"].isin(
            ["BE", "CZ", "DE", "DK1", "DK2", "NL", "NO1", "NO3", "NO4", "PL", "SE1", "SE2", "SE3", "SE4", "UK",
             "FI"])].dropna(axis=1).reset_index(drop=True)
        ...
        tyndp_demand = pd.read_csv(path + "/TYNDP/load_tyndp2020-v02-al-2022_08_15.csv").dropna(
            axis=1)  # [["y", "t", "BE", "CZ", "DE", "DK1", "DK2", "NL", "NO1", "NO3", "NO4", "PL", "SE1", "SE2", "SE3", "SE4", "UK", "FI"]]
        columns = pd.Series(tyndp_demand.columns.str.split("00", expand=True).get_level_values(0))
        columns = columns.replace(
            {"DKE1": "DK1", "DKW1": "DK2", "NOM1": "NO3", "NON1": "NO4", "NOS0": "NO1", "SE01": "SE1", "SE02": "SE2",
             "SE03": "SE3", "SE04": "SE4"})
        tyndp_demand.columns = columns
        tyndp_demand = tyndp_demand[
            ["YEAR", "BE", "CZ", "DE", "DK1", "DK2", "NL", "NO1", "NO3", "NO4", "PL", "SE1", "SE2", "SE3", "SE4", "UK",
             "FI"]]
        tyndp_demand_years = {
            i: tyndp_demand[tyndp_demand["YEAR"] == year].reset_index(drop=True).drop(["YEAR"], axis=1) for year, i in
            zip([2030, 2040], [0, 2])}
    ...
    def extend_overloaded_lines(self, type, case_name):
        ...
        try:
            overloaded_AC_lines = \
            pd.read_csv("results/" + case_name + "/1/subscen0/overloaded_lines_AC.csv", index_col=0)[
                "full_load_h"].to_dict()
            overloaded_DC_lines = \
            pd.read_csv("results/" + case_name + "/1/subscen0/overloaded_lines_DC.csv", index_col=0)[
                "full_load_h"].to_dict()
        except:
            sys.exit("need to run scenario 1 first!")
            ...
            case "AC":
                for key in self.ac_lines.index:
                    if key in overloaded_AC_lines.keys():
                        self.ac_lines.loc[key, "max"] = self.ac_lines.loc[key, "max"] * (
                                    1 + 0.2 * overloaded_AC_lines[key] / (
                                        self.number_years * self.timesteps_reduced_ts))
            case "DC":
                for key in self.dc_lines.index:
                    if key in overloaded_DC_lines.keys():
                        self.dc_lines.loc[key, "max"] = self.dc_lines.loc[key, "max"] * (
                                    1 + 0.2 * overloaded_DC_lines[key] / (
                                        self.number_years * self.timesteps_reduced_ts))
