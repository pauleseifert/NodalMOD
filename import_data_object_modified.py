...
class run_parameter:
    def __init__(self, scenario_name):
        ...
        #
        self.hours = 24 #21 representative days
...
class model_data:
    def __init__(self, create_res,reduced_ts, export_files, run_parameter):
        ...
        if create_res:
            ...
            busses_raw = busses_raw.drop(379).reset_index()
            ...
            generators_matched = generators_matched[generators_matched['node'].notnull()]
            ...
            #lines_filtered["index_x"] = lines_filtered["index_x"].astype(int)
            #lines_filtered["index_y"] = lines_filtered["index_y"].astype(int)
            ...
            #self.ac_lines = lines
            self.ac_lines = lines.reset_index(drop=True)
        ...
        self.PTDF = self.Calculate_PTDF(nodes = self.nodes, ac_lines = self.ac_lines)

    def Calculate_PTDF(self, nodes, ac_lines):
        #nodes = self.nodes
        #ac_lines = self.ac_lines
        ...
        nb = nodes.shape[0]
        nbr = ac_lines.shape[0]
        NN = nodes.reset_index()

        b = 1 / ac_lines.loc[:, ["x"]].to_numpy()  ## series susceptance
        f = ac_lines.loc[:, ["from"]].to_numpy()
        t = ac_lines.loc[:, ["to"]].to_numpy()

        Cftp = np.zeros([nb, nbr])
        for i in range(nbr):
            Cftp[np.where(NN["index"] == f[i][0]), i] = 1
            Cftp[np.where(NN["index"] == t[i][0]), i] = -1

        Bf = np.zeros([nbr, nb])
        for i in range(nbr):
            Bf[i, np.where(NN["index"] == f[i][0])] = b[i]
            Bf[i, np.where(NN["index"] == t[i][0])] = -b[i]

        Bbus = np.matmul(Cftp, Bf)

        #PTDF = np.zeros([nbr, nb])
        #noref = np.linspace(1, nb, nb - 1, endpoint=False, dtype=int)
        noslack = np.delete(NN.index.values, NN.loc[NN["bus_id"] == "Nordel: 9024"].index[0])

        #PTDF = Bf[:, noref] @ np.linalg.inv(Bbus[:, noref][noslack, :])
        PTDF = Bf[:, noslack] @ np.linalg.inv(Bbus[:, noslack][noslack, :])
        PTDF = np.insert(PTDF,NN.loc[NN["bus_id"] == "Nordel: 9024"].index[0], 0, axis=1)
    ...
    def resolve_bidding_zones(self):
        ...
        self.nodes = nodes_scand_bidding_zones_resolved
