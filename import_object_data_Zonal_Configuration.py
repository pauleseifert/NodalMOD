...
class model_data:
    def __init__(self, create_res, reduced_ts, export_files, run_parameter):
        ...
        match run_parameter.scen:
            case"BAU":self.ntc = pd.read_excel("data\\final_readin_data\\ntc_BAU_einlesen.xlsx")
            case"BZ2":self.ntc = pd.read_excel("data\\final_readin_data\\ntc_BZ2_einlesen.xlsx")
            case"BZ3":self.ntc = pd.read_excel("data\\final_readin_data\\ntc_BZ3_einlesen.xlsx")
            case"BZ5":self.ntc = pd.read_excel("data\\final_readin_data\\ntc_BZ5_einlesen.xlsx")
...
class kpi_data:
 ...
 #           share_renewables = self.change_column_to_int(share_renewables)
        ...
        zones_raw = self.read_in(y="", string="zones.csv", int_convert=False)
